...
simulate_main <- function(sim_i, results_input, ploidytest_input) {
  # run gene expression simulation
  sim_counts_stratified <- lapply(unique(results_input$cell_group), function(x) sim_by_celltype(results, ploidytest_input, x))
  ...
  simulated_ase <- rbind(simulate_allelic_ratio(results_input, 1, n = nrow(results_input[ploidy == 1])),
                         simulate_allelic_ratio(results_input, 2, n = nrow(results_input[ploidy == 2])),
                         simulate_allelic_ratio(results_input, 3, n = nrow(results_input[ploidy == 3])))
  
  monosomy_ar <- simulate_allelic_ratio(results_input, 1, n = nrow(scploid_sim_results[monosomy == TRUE]))
  disomy_ar <- simulate_allelic_ratio(results_input, 2, n = nrow(scploid_sim_results[is.na(monosomy)]))
  trisomy_ar <- simulate_allelic_ratio(results_input, 3, n = nrow(scploid_sim_results[monosomy == FALSE]))
...
set.seed(12983)
sim <- do.call(rbind, pbmclapply(1:100, 
                                 function(x) simulate_main(x, results, ploidytest_dt), 
                                 mc.cores = getOption("mc.cores", 24L)))

group_by(sim, signature, nom_fdr) %>%
  summarize_all(list(mean)) %>%
  select(-sim_index) %>%
  arrange(nom_fdr, signature)
